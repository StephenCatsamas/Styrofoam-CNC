...
const uint32_t maxPos[] = {1251, 35001, 4000000};            // Right most encoder boundary  // MEASURE THIS
...
const float coP[] = {0.1,2,0.0001};                     //PID coefficients for each axis adjust these to taste.
const float coI[] = {0,0.01,0};
const float coD[] = {-2,0.00,0.001};

volatile int8_t encPhaseALast[] = {LOW, LOW, LOW}; 
volatile bool reached[] ={false,false,false};                 // true if at tgt
...
volatile bool dir[] = {0,0,0};                            //motor direction, any semblance to actual movement in purely coincidental
...
volatile int32_t PID[] = {0,0,0};                     //resultant PID value after calling in one go_to_tgt
volatile float Perr[] = {0,0,0};                      //PID calulation internal use only
volatile float Ierr[] = {0,0,0};
volatile float Derr[] = {0,0,0};                      
volatile uint8_t count[] = {0,0,0};                   //counts how many to_tgts for occational zeroing
...
volatile int32_t point_0[] = {0,0,0};             //for use in random functions
volatile int32_t point_1[] = {0,0,0};
volatile int32_t loopvar[] = {0,0,0};
volatile int32_t v;
volatile int8_t i;
...
void loop() {
    ...
    PID[Th] = doPID(Th);
    Serial.print(Perr[Th]);
    Serial.print(" ");
    Serial.print(Ierr[Th]);
    Serial.print(" ");
    Serial.println(Derr[Th]);
