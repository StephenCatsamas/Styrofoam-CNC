...
int16_t PID[] = {0,0};
const float coP[] = {0.01,0.04};
const float coI[] = {0.0000001,0};
const float coD[] = {0.2,0.01};
float Perr[] = {0,0};
float Ierr[] = {0,0};
float Derr[] = {0,0};
int8_t off[] = {-100,10};
...
void go_to_tgt(uint8_t axis) {
  ...
  while (reached == false){
      ...
      if(millis() - timetgt[axis] > 1000){ //if taking too long it get a jolt
          motor[axis]->setSpeed(250); 
          motor[axis]->forward(); 
          delay(100); 
          motor[axis]->stop();
...
float doPID(uint8_t axis){
   ...
   listShift(time);
    ...
    errFncR[0] = tgt[R] - pos[R]+off[axis];
    errFncZ[0] = tgt[Z] - pos[Z]+off[axis];
    time[0] = millis();
