...
L298N motorC(ENTh, IN1Th, IN2Th);
L298N motorD(ENQ, IN1Q, IN2Q);
L298N* motor[] = { &motorA, &motorB, &motorC, &motorD};
...
const float coP[] = {0.1,2,1};                     //PID coefficients for each axis adjust these to taste.
const float coI[] = {0,0.01,0};
const float coD[] = {-2,0.00,0};
...
void go_to_tgt(uint8_t axis) { //R axis positioning is only prescise on extension //only use this method for manual i-know-what-i-am-doing positioning.
    ...
    if(axis == Th){
        ...
        if(PID[Th] > 0){                             //normal PID control
            if(120 + PID[Th] > 255){
                motor[Th]->setSpeed(255);
            }else{
                motor[Th]->setSpeed(140 + PID[Z]);
            }
        ...
        if(pos[Th] - tgt[Th] <= 2 && pos[Th] - tgt[Th] >= -2){ //checks if at tgt
            reached[Th] = true;
            motor[Th]->stop();
            delay(200);
        }
