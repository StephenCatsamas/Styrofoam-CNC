...
int8_t encPhaseALast[] = {LOW, LOW, LOW};
...
void loop() {
  ...
  //cut_sphere( 50,  1, 0, 12);
    /* Serial.print(digitalRead(encTheta[0]));
    Serial.print(" ");
    Serial.println(digitalRead(encTheta[1])); */
    Serial.println(pos[Th]);
...
void go_to_tgt(uint8_t axis) { //R axis positioning is only prescise on extension //only use this method for manual i-know-what-i-am-doing positioning.
  if (count[Z] == 20 || dist[Z] > 120000 || (pos[Z] > 20000 && tgt[Z] - pos[Z] < 0)){
  ...
  if(axis == R){
   ...
   if(axis == Z){
      ...
      if(millis() - timetgt[axis] > 1000 && axis == R){ //if taking too long it get a jolt
          motor[axis]->stop();
          motor[axis]->setSpeed(255); 
          motor[axis]->forward(); 
          delay(100); 
